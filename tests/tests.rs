...
mod test_functions {
    ...
    fn test_odd_to_list_gen1to_num_functions_combined() {
        assert_eq!(t("odd { dup 2 div swap 2 / == if False True } := \
...
// "odd { dup 2 div swap 2 / == if False True } := toList { [ ] swap times cons } fun gen1toNum { max swap := 1 loop { dup max > } { dup 1 + } } fun 4 gen1toNum 5 toList map odd"), "[True,False,True,False,True]");
//
//
//
